...
def get_item(x, y):
    for sensor in sensors:
        if sensor['x'] == x and sensor['y'] == y:
            return 'S'
        elif sensor['b']['x'] == x and sensor['b']['y'] == y:
            return 'B'
        elif no_beacon.get(f'{x}_{y}'):
            return '#'
...
def mark_no_beacon(sensor, d, max):
...
def compute_coverage(sensor, max):
    d = sensor_beacon_dist(sensor)
    mark_no_beacon(sensor, d,max)

load_map('test.txt')
max=4000000
for sensor in sensors:
    compute_coverage(sensor,max)
